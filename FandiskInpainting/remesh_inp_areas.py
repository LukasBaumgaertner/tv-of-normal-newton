...
mesh = Mesh("../meshes/fandisk.xml")
mesh.init()    

CG = FunctionSpace(mesh, "CG", 1)
myfun = Function(CG)
vec = myfun.vector().get_local()


vertlist = [5713,5844] #6075
...
move = Function(VCG)
for i in range(4000):
    move.vector().set_local(-1e-2*assemble( div(V)*dx).get_local()*np.repeat(vec2,3))
    ALE.move(mesh, move)
...
os.system("$GMSH ./remesh_fandisk.geo -2 -o remeshed_fandisk.msh")

os.system("dolfin-convert remeshed_fandisk.msh remeshed_fandisk.xml")

os.system('cat remeshed_fandisk.xml | sed "s/dim=\\"2\\"/dim=\\"3\\"/" > remeshed_fandisk_fixed.xml')
os.system("rm remeshed_fandisk.xml")
os.system("mv remeshed_fandisk_fixed.xml remeshed_fandisk.xml")
